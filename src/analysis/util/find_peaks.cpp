...
static void diff(std::vector<float> in, std::vector<float>& out)
{
...
static void vectorProduct(std::vector<float> a, std::vector<float> b, std::vector<float>& out)
{
...
static void findIndicesLessThan(std::vector<float> in, float threshold, std::vector<int>& indices)
{
...
static void selectElements(std::vector<float> in, std::vector<int> indices, std::vector<float>& out)
{
...
static void signVector(std::vector<float> in, std::vector<int>& out, int sign)
{
...
std::vector<int> Analysis::findPeaks(const float *data, int length, int sign)
{
    std::vector<float> x0(data, std::next(data, length));
    ...
    if(len>2)
    {
        ...
        std::vector<float> xSub0(x.begin(), x.begin()+3);//tener cuidado substd::vector
        std::vector<float> xDiff;//tener cuidado substd::vector
        diff(xSub0, xDiff);
