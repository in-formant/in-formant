...
int Wt::CAT(TYPE, _downcoef_axis)(const TYPE * const restrict input, const ArrayInfo input_info,
    ...
    if (make_temp_input)
        if ((temp_input = (TYPE *) malloc(input_info.shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
    if (make_temp_output)
        if ((temp_output = (TYPE *) malloc(output_info.shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
...
int Wt::CAT(TYPE, _idwt_axis)(const TYPE * const restrict coefs_a, const ArrayInfo * const a_info,
    ...
    if (make_temp_coefs_a)
        if ((temp_coefs_a = (TYPE *) malloc(a_info->shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
    if (make_temp_coefs_d)
        if ((temp_coefs_d = (TYPE *) malloc(d_info->shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
    if (make_temp_output)
        if ((temp_output = (TYPE *) malloc(output_info.shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
...
int Wt::CAT(TYPE, _dec_a)(const TYPE * const restrict input, const size_t input_len,
                      const DiscreteWavelet * const restrict wavelet,
...
int Wt::CAT(TYPE, _dec_d)(const TYPE * const restrict input, const size_t input_len,
                      const DiscreteWavelet * const restrict wavelet,
                      TYPE * const restrict output, const size_t output_len,
                      const Mode mode){
...
int Wt::CAT(TYPE, _rec_a)(const TYPE * const restrict coeffs_a, const size_t coeffs_len,
                      const DiscreteWavelet * const restrict wavelet,
                      TYPE * const restrict output, const size_t output_len){
...
int Wt::CAT(TYPE, _rec_d)(const TYPE * const restrict coeffs_d, const size_t coeffs_len,
                      const DiscreteWavelet * const restrict wavelet,
                      TYPE * const restrict output, const size_t output_len){
 ...
 */
int Wt::CAT(TYPE, _idwt)(const TYPE * const restrict coeffs_a, const size_t coeffs_a_len,
...
int Wt::CAT(TYPE, _swt_)(const TYPE * const restrict input, int input_len,
    ...
    if(level > 1){
        ...
        e_filter.resize(e_filter_len, REAL_TYPE(0.0));
 ...
 */
int Wt::CAT(TYPE, _swt_a)(const TYPE * const restrict input, int input_len,
                      const DiscreteWavelet * const restrict wavelet,
                      TYPE * const restrict output, int output_len,
                      unsigned int level){
    return CAT(TYPE, _swt_)(input, input_len, wavelet->dec_lo,
 ...
 * input - approximation coeffs from upper level or signal if level == 1
 */
int Wt::CAT(TYPE, _swt_d)(const TYPE * const restrict input, int input_len,
                      const DiscreteWavelet * const restrict wavelet,
                      TYPE * const restrict output, int output_len,
                      unsigned int level){
    return CAT(TYPE, _swt_)(input, input_len, wavelet->dec_hi,
                            wavelet->dec_len, output, output_len, level);
}
