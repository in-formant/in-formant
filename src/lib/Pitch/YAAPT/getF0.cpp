...
void YAAPT::getF0_slow(const AudioFrames & data, double fs, Result & res, const Params & prm)
{
    ...
    if (lenTemporal < lenSpectral) {
        ...
        tPitch1_pad << tPitch1, ArrayXXd::Zero(maxCands, lenSpectral - lenTemporal);
        tPitch1 = tPitch1_pad;

        ArrayXXd tMerit1_pad(maxCands, lenSpectral);
        tMerit1_pad << tMerit1, ArrayXXd::Zero(maxCands, lenSpectral - lenTemporal);
        tMerit1 = tMerit1_pad;

        ArrayXXd tPitch2_pad(maxCands, lenSpectral);
        tPitch2_pad << tPitch2, ArrayXXd::Zero(maxCands, lenSpectral - lenTemporal);
        tPitch2 = tPitch2_pad;

        ArrayXXd tMerit2_pad(maxCands, lenSpectral);
        tMerit2_pad << tMerit2, ArrayXXd::Zero(maxCands, lenSpectral - lenTemporal);
        tMerit2 = tMerit2_pad;
    }
