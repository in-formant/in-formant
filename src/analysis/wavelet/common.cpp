...
#error "Unrecognized SIZE_MAX"
    ...
    int leading_zeros = (int) __builtin_clz(x);
#elif SIZE_MAX == ULONG_MAX
    int leading_zeros = (int) __builtin_clzl(x);
#elif SIZE_MAX == ULLONG_MAX
    int leading_zeros = (int) __builtin_clzll(x);
#else
#error "Unrecognized SIZE_MAX"
#endif /* SIZE_MAX */
...
size_t Wt::dwt_buffer_length(size_t input_len, size_t filter_len, Mode mode) {
...
size_t Wt::reconstruction_buffer_length(size_t coeffs_len, size_t filter_len){
...
size_t Wt::idwt_buffer_length(size_t coeffs_len, size_t filter_len, Mode mode){
    switch(mode){
            case MODE_PERIODIZATION:
                return 2*coeffs_len;
            default:
...
size_t Wt::swt_buffer_length(size_t input_len){
    return input_len;
}

int Wt::dwt_max_level(size_t input_len, size_t filter_len){
...
int Wt::swt_max_level(size_t input_len){
